...
namespace WebStore.Services.InSQL
{
    public class SqlEmployeesData : IEmployeesData
    {
        ...
        {
            //return _db.Employees.FirstOrDefault(e => e.Id == id); запрос к БД всегда
            return _db.Employees.Find(id); //сначала проверяется кэш
        }
        ...
        public bool Delete(int id)
        {
            ...
            var employee = _db.Employees
               .Select(e => new Employee { Id = e.Id, }) // Неполная проекция - для экономии памяти и времени на передачу данных (у объекта заполняется только id)
